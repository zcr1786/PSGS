...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, output1, output2, label):
        ...
        loss_contrastive = torch.mean((1-label) * torch.pow(xxl-0.5, 2) +
                                      (label) * torch.pow(torch.clamp(self.margin - xxl, min=0.0), 2))
        # loss_contrastive = torch.mean((1 - label) * (xxl - 0.5) +
        #                               (label) * (1 - xxl))
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.output_layer = models.classifier(self.latent_size * 2 , self.num_labels, self.device)
    ...
    def train(self, graph_index_x,graph_index_y, epoch):
        ...
        simloss1 = self.sia_loss(mygrax, myseqx, 0)
        simloss2 = self.sia_loss(mygray, siaseqx, 1)
        simloss3 = self.sia_loss(siagrax, myseqy, 1)
        simloss4 = self.sia_loss(graymy, seqymy, 0)
        ...
        label = torch.tensor(self.train_labels[graph_index_x]).to(self.device)
        ...
        loss = label_loss  +( simloss1 +( simloss2 + simloss3  )*0.5 )
